...
-- ============================================================================
-- SYNTHETIC DATA
-- Scenario: Gujarat Earthquake Relief (2-Day Demo)
--
-- USERS:   1 Admin, 2 Camp Managers (C1, C2), 3 Drivers
-- CAMPS:   C1 manages 3 camps (north cluster), C2 manages 2 camps (south cluster)
-- 
-- DAY 1:  Initial requests — high urgency, first delivery cycle
-- DAY 2:  Follow-up requests — replenishment + new emergency camp request
--
...
-- id=2  CAMP MANAGER 1 (manages 3 camps — north cluster)
('Priya Sharma',      'cm1@fairshare.org',      '9876543211', 'camp_manager',
 'scrypt:32768:8:1$XKxjF3qLmN7yTBvR$a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4'),

-- id=3  CAMP MANAGER 2 (manages 2 camps — south cluster)
...
-- camp_id=1  NORTH — critical
('Camp Alpha',   200, 750,  850, 600,  0.75, 'critical',  2, NOW() - INTERVAL '2 days'),
-- camp_id=2  NORTH — moderate
('Camp Bravo',   350, 800,  400, 150,  0.38, 'moderate',  2, NOW() - INTERVAL '2 days'),
-- camp_id=3  NORTH — moderate (upper-mid)
('Camp Charlie', 150, 650,  620, 400,  0.64, 'moderate',  2, NOW() - INTERVAL '2 days'),
-- camp_id=4  SOUTH — critical
('Camp Delta',   700, 300,  950, 750,  0.84, 'critical',  3, NOW() - INTERVAL '2 days'),
-- camp_id=5  SOUTH — moderate
('Camp Echo',    800, 200,  300, 100,  0.32, 'moderate',  3, NOW() - INTERVAL '2 days');
...
-- ── Camp Alpha (urgency=0.75 → critical) ──
...
-- ── Camp Delta (urgency=0.84 → critical) ──
INSERT INTO requests (camp_id, item_type, quantity_needed, priority, status, request_date) VALUES
